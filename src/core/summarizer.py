...
class RepoSummarizer:
    ...
    def __init__(
        ...
        self.generator = generator or LLMGenerator(
            prompt_builder=self.prompt_builder, debug=self.debug
        )
    ...
    async def summarize(self, repo_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if len(readme) > 10000:
            ...
            logger.debug(
                f"Truncated README from {original_readme_length} to 10000 chars"
            )
        ...
        try:
            ...
            if self.debug:
                ...
                logger.debug(
                    f"Generated prompt (length: {len(prompt)} chars):\n{prompt}"
                )
    ...
    async def summarize_batch(
        ...
        """
        logger.info(
            f"Summarizing {len(repos)} repositories with concurrency={self.concurrency}"
        )
            ...
            if output_file.exists():
                try:
                    ...
                    if isinstance(existing_data, list):
                        logger.info(
                            f"Loaded {len(existing_data)} existing repositories from {output_file}"
                        )
        ...
        for repo in repos:
            ...
            if repo_name in repo_dict:
                ...
                has_summary = (
                    existing_repo.get("summary")
                    and isinstance(existing_repo.get("summary"), str)
                    and len(existing_repo.get("summary", "")) > 10
                )
                has_tags = (
                    existing_repo.get("tags")
                    and isinstance(existing_repo.get("tags"), list)
                    and len(existing_repo.get("tags", [])) > 0
                )
                has_error = "error" in existing_repo

                if has_summary and has_tags and not has_error:
                    logger.info(
                        f"Skipping {repo_name}: already exists in output file with valid summary and tags"
                    )
                    already_processed.append(existing_repo)
                    continue
                elif has_error:
                    logger.info(
                        f"Reprocessing {repo_name}: previous attempt had error: {existing_repo.get('error')}"
                    )
            ...
            if has_summary and has_tags and not has_error:
                ...
                if has_error:
                    logger.info(
                        f"Reprocessing {repo_name}: has error: {repo.get('error')}"
                    )
                else:
        ...
        logger.info(
            f"Found {len(already_processed)} already processed repositories, processing {len(to_process)} new or error ones"
        )
